class BinHeap(object):
    ...
    def perc_up(self, i):
        """
        Recursively switch the parent node with current if the current is smaller
        """
        while i // 2 > 0:
            upval = i // 2
    ...
    def insert(self, k):
        """
        Insert new value at the end of the binary heap and recursively perc it
        upwards until it is in the appropriate spot.
        """
    ...
    def perc_down(self, i):
        """
        Recursively switch the parent node with current if the current is bigger
        """
        while i * 2 <= self.size:
    ...
    def min_child(self, i):
        """
        Return the child with a lower value. If there is one child, return that.
        """
        try:
